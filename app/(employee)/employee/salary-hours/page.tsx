...
import { DailyWork, Employee, getEmployeeMonthlyData } from "../employeeactions";
...
export default function EmployeeSalaryPage() {
  ...
  return (
    <div className="p-6 mt-20 max-w-7xl mx-auto space-y-6">
      ...
      <p className="text-gray-600 mb-4">Monthly overview of login/logout times and salary.</p>
          ...
          <div className="bg-white rounded-lg shadow p-4">
            ...
            <div className="grid grid-cols-7 gap-1">
              {Array.from({ length: daysInMonth }, (_, i) => {
                ...
                return (
                  <div key={i} className={`h-24 border rounded p-1 flex flex-col justify-between items-center ${logs.length ? "bg-yellow-50" : "bg-gray-50"}`}>
                    <span className="font-semibold">{day}</span>
                    {logs.length ? (
                      logs.map((log, idx) => (
                        <div key={idx} className="text-sm flex flex-col items-center">
                          <span className="flex items-center gap-1">
                            Login: <Clock className="w-4 h-4 text-teal-600" /> {formatTime(log.loginTime)}
                          </span>
                          <span className="flex items-center gap-1">
                            Logout: <Clock className="w-4 h-4 text-teal-600" /> {formatTime(log.logoutTime)}
                          </span>
                          <span className="text-xs text-gray-600">{(log.totalMinutes / 60).toFixed(2)} h</span>
                        </div>
                      ))
                    ) : (
